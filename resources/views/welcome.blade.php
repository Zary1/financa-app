    ...
    <p class="text-white text-xl text-center pb-9">{{Auth::guard('web')->user()->name}}</p>
     ...
     <div class="space-y-9 pl-9">
        <p class="text-white text-xl cursor-pointer hover:text-cold">
          <i class="fas fa-wallet w-6 h-6 inline-block mr-2"></i> Saldo atual</p>
        <p class="text-white text-xl cursor-pointer hover:text-cold">
...
<div id="overlay" class="fixed left-1/4 inset-x-0  bg-black bg-opacity-30 hidden flex items-center justify-center h-screen">
...
<!-- div goals -->
<div id="goals" class="fixed left-1/4 inset-x-0 hidden bg-black bg-opacity-30  flex items-center justify-center h-screen">
...
<form action="/create-goal" method="POST" class="space-y-4 p-6">
    ...
    <input type="number" id="goal_amount" name="goal_amount" placeholder="Valor disponível (€):"
    required step="0.01" class="w-full p-2 rounded-lg text-black  border-cold ">
...
<!-- alterar senha -->
<div id="senha" class="fixed left-1/4 hidden inset-x-0 bg-black bg-opacity-30 flex items-center justify-center h-screen">
    <div class="bg-white text-black pl-6 pt-4 rounded-lg shadow-lg w-[600px]">
    ...
    <p class="text-cold">{{ session('sucess') }}</p>
        ...
        <div id="close_senha" class="flex cursor-pointer items-center justify-center w-[50px] h-[30px] ml-[500px] bg-purple rounded-full">
            <i class="fa-solid fa-delete-left text-red-500 text-lg"></i>
        </div>
